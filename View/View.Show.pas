...
type
  TView = class
    ...
    //class procedure ExibirDadosDoProduto( aProduto
    ...
    class procedure ExibirAvisoDePedidoNovo( aValue: boolean );
...
begin
  ...
  ExibirAvisoDePedidoNovo( false );
end;

class procedure TView.ExibirAvisoDePedidoNovo(aValue: boolean);
begin
  frmPedidosDeVenda.lbPedidoNovo.Visible := aValue;
end;
...
begin
  ...
  frmPedidosDeVenda.pnTotalDoPedido.Caption := Format(' Total do Pedido Nr %d:   R$ ' + FormatFloat('#,##0.00', aPedido.Valor_total ),[ aPedido.Numero_pedido ]);
end;
